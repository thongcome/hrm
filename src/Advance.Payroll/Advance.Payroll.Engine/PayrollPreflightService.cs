...
//
// เดิม: context.Hremployee โดยตรง (eligible query + Hold lookups) — ตอนนี้ผ่าน IEmployeeSource
// TODO(seam): the pending-OT checklist item read HRM's `emp_overtime_requests` table
// directly (a DIFFERENT table from HrwOt/IOvertimeFeed — this preflight check predates
// the feed split and was never updated to match). Not one of the 7 Contracts interfaces;
// left out entirely here rather than guessed at — see EXTRACTION-PLAN.md "seams beyond
// the original 7". The adhoc-item and salary-advance checklist items are unaffected
// (both are Payroll's own tables).
public class PayrollPreflightService(IDbContextFactory<PayrollDbContext> dbFactory, IEmployeeSource employeeSource)
{
    ...
    public async Task<Report> CheckAsync(long runId, CancellationToken ct = default)
    {
        ...
        var eligible = await employeeSource.GetEligibleEmployeesAsync(run.CompanyId, run.PeriodStart, run.PeriodEnd, ct);

        var issues = new List<Issue>();
        foreach (var e in eligible.Where(e => !heldIds.Contains(e.HremployeeId)))
        {
            var name = $"{e.EmpName} {e.EmpSurname}".Trim();
            if ((e.SalaryAmt ?? 0m) <= 0m && (e.DailyWage ?? 0m) <= 0m)
                issues.Add(new Issue(e.HremployeeId, e.EmpNo, name, Severity.Error, "NO_SALARY", "ไม่มีเงินเดือนฐานและไม่มีค่าจ้างรายวัน — คำนวณได้ 0 บาท"));
            if (string.IsNullOrWhiteSpace(e.SalexpAccid))
                issues.Add(new Issue(e.HremployeeId, e.EmpNo, name, Severity.Warning, "NO_BANK_ACCOUNT", "ไม่มีเลขบัญชีธนาคาร — คำนวณได้ แต่ต้องเติมก่อนทำไฟล์โอนเงิน"));
            else if (string.IsNullOrWhiteSpace(e.SalexpBank))
                issues.Add(new Issue(e.HremployeeId, e.EmpNo, name, Severity.Warning, "NO_BANK_CODE", "มีเลขบัญชีแต่ไม่ระบุธนาคาร"));
            if (string.IsNullOrWhiteSpace(e.CostCenterCode))
                issues.Add(new Issue(e.HremployeeId, e.EmpNo, name, Severity.Warning, "NO_COST_CENTER", "ไม่มีศูนย์ต้นทุน — GL/รายงานต้นทุนจะไม่มีที่ลง"));
            if (string.IsNullOrWhiteSpace(e.PosCode))
                issues.Add(new Issue(e.HremployeeId, e.EmpNo, name, Severity.Warning, "NO_POSITION", "ไม่มีตำแหน่ง — เบี้ยตามตำแหน่งจะไม่ถูกจ่าย"));
        }

        var nameById = eligible.ToDictionary(e => e.HremployeeId, e => (e.EmpNo, Name: $"{e.EmpName} {e.EmpSurname}".Trim()));
        ...
        // TODO(seam): pending-OT checklist item dropped — see class header comment (emp_overtime_requests has no seam).
    ...
    public async Task HoldAsync(long runId, long hremployeeId, string reason, long actorUserId, CancellationToken ct = default)
    {
        ...
        var empNo = (await employeeSource.GetByIdsAsync(new[] { hremployeeId }, ct)).GetValueOrDefault(hremployeeId)?.EmpNo;
    ...
    {
        // เดิม: query ctx.Hremployee ตรง ๆ — ตอนนี้โหลดผ่าน IEmployeeSource ทั้งบริษัทแล้วกรอง (ไม่มี GetByEmpNo ใน interface)
        var trimmed = empNo.Trim();
        var eligible = await employeeSource.GetEligibleEmployeesAsync(companyId, DateOnly.MinValue, DateOnly.MaxValue, ct);
        var id = eligible.FirstOrDefault(e => e.EmpNo == trimmed)?.HremployeeId;
