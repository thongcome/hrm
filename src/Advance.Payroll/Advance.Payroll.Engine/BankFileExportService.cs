...
public class BankFileExportService
{
    ...
    public BankFileExportService(IDbContextFactory<PayrollDbContext> dbFactory, IEmployeeSource employeeSource, PrivateFileStorage fileStorage)
    {
    ...
    public async Task<long> ExportAsync(long runId, long actorUserId, CancellationToken ct = default)
    {
        ...
        var seqNo = 0;
        var lineValues = lines.Select(l =>
        {
            var p = people.GetValueOrDefault(l.Emp.HremployeeId);
            return BankFileValues.ForLine(++seqNo, l.Emp.EmpNo, p?.EmpName, p?.EmpSurname,
                l.Emp.BankCode, l.Emp.BankBranchCode, l.Emp.BankAccountNo, l.Amount, p?.IdCard, p?.AdnEmail);
        }).ToList();
    ...
    private static async Task CreateRecoveryItemsAsync(PayrollDbContext context, Pay_PayrollRun run, Pay_PayrollRun original,
