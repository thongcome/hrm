...
{
    // เดิม: อ่านชื่อจาก employee.Hremployee.EmpName/EmpSurname — ตอนนี้ผู้เรียกส่ง employeeName มา
    // (resolve ผ่าน IEmployeeSource ก่อนเรียก; ดู PayslipGenerationService.cs)
        ...
        string password)
    {
        var plainBytes = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                {
                    column.Item().Text($"รหัสพนักงาน: {employee.EmpNo}    ชื่อ: {employeeName}");
