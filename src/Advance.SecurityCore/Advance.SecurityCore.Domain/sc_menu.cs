...
// Copied from HRM's Model/sc_menu.cs. Trim: `menugroup` navigation to
// sc_menugroup removed — sc_menugroup is a single-row legacy lookup table
// (every existing row uses menugroupid=1) that wasn't in this extraction's
// requested entity list. menugroupid stays a plain scalar column so the FK
// shape on the live table is unchanged.
[Table("sc_menu")]
